...
import { revisar } from '@core/srs/sm2';
import { progressoInicial } from '@core/types/schema';
async function registrarDescobertaSeDominouAgora(antes, depois, resumo_id) {
    if (antes === 'dominado' || depois !== 'dominado')
        return;
    try {
...
async function obter(cache, resumo_id) {
    if (cache[resumo_id])
        return cache[resumo_id];
...
export const useProgressoStore = create((set, get) => ({
    cache: {},
    ...
    revisarBloco: async (resumo_id, q, confianca) => {
        ...
        if (statusAntes !== 'dominado' && p.srs.status === 'dominado') {
            ...
            if (typeof window !== 'undefined')
                window.dispatchEvent(new Event('anima:rituais'));
        }
        else {
    ...
    desfazerUltimaRevisao: async (resumo_id) => {
        ...
        if (ultimo.alvo === 'bloco')
            p.srs = estadoRestaurado;
        else
            p.srs_cards = { ...p.srs_cards, [ultimo.alvo]: estadoRestaurado };
        await persistir(p);
        set((s) => ({ cache: { ...s.cache, [resumo_id]: p } }));
    },
