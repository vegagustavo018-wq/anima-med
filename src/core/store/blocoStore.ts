...
interface BlocoState {
  blocoAtual: BlocoConteudo | null
  carregando: boolean
  erro: string | null

  carregarBloco: (id: string) => Promise<BlocoConteudo | null>
  previewsPorPrefixo: (prefixo: string) => Promise<BlocoPreview[]>
  previewsPorDisciplina: (disciplina: string) => Promise<BlocoPreview[]>
  contarPorPrefixo: (prefixo: string) => Promise<number>
  buscar: (termo: string) => Promise<BlocoPreview[]>
}
...
export const useBlocoStore = create<BlocoState>((set) => ({
  blocoAtual: null,
  carregando: false,
  erro: null,
