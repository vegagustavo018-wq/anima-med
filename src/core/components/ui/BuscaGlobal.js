...
export function BuscaGlobal() {
    ...
    const { toggleFoco, toggleSidebar, setReduzirMovimento, reduzirMovimento, setPerfilSessao } = useUIStore();
    ...
    const comandosFiltrados = useMemo(() => {
        ...
        return comandos.filter((c) => c.label.toLowerCase().includes(t) || c.palavrasChave?.toLowerCase().includes(t));
    }, [termo, comandos]);
    useEffect(() => {
        const onKey = (e) => {
    ...
    if (!aberto)
        return null;
    return (_jsx("div", { onClick: () => setAberto(false), style: {
            position: 'fixed',
            inset: 0,
            ...
            animation: 'entrarSuave 0.15s ease',
        }, children: _jsxs("div", { role: "dialog", "aria-modal": "true", "aria-label": "Central de comando: buscar blocos ou executar a\u00E7\u00F5es", onClick: (e) => e.stopPropagation(), style: {
                width: '100%',
                maxWidth: 640,
                ...
                animation: 'entrarBaixo 0.18s ease',
            }, children: [_jsx("input", { autoFocus: true, value: termo, "aria-label": "Buscar blocos ou comandos", role: "combobox", "aria-expanded": totalItens > 0, "aria-controls": "busca-resultados", onChange: (e) => setTermo(e.target.value), onKeyDown: (e) => {
                        if (e.key === 'ArrowDown')
                            setSel((s) => Math.min(totalItens - 1, s + 1));
                        if (e.key === 'ArrowUp')
                            setSel((s) => Math.max(0, s - 1));
                        if (e.key === 'Enter') {
                            if (sel < comandosFiltrados.length)
                                executar(comandosFiltrados[sel]);
                            else {
                    ...
                    }, placeholder: "Buscar blocos ou comandos\u2026", style: {
                        width: '100%',
                        padding: '18px 20px',
                        border: 'none',
                        ...
                        boxSizing: 'border-box',
                    } }), _jsx("div", { "aria-live": "polite", style: { position: 'absolute', width: 1, height: 1, overflow: 'hidden', clip: 'rect(0 0 0 0)', whiteSpace: 'nowrap' }, children: termo.trim() ? `${totalItens} resultado${totalItens === 1 ? '' : 's'}` : '' }), totalItens > 0 && (_jsxs("div", { id: "busca-resultados", role: "listbox", "aria-label": "Resultados", style: { maxHeight: '55vh', overflowY: 'auto' }, children: [comandosFiltrados.length > 0 && (_jsx("div", { style: { padding: '8px 20px 4px', fontSize: 10, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--color-text-muted)' }, children: "Comandos" })), comandosFiltrados.map((c, i) => (_jsxs("button", { onClick: () => executar(c), onMouseEnter: () => setSel(i), style: {
                                width: '100%',
                                display: 'flex',
                                alignItems: 'center',
                                gap: 12,
                                ...
                                cursor: 'pointer',
                            }, children: [_jsx("span", { style: { fontSize: 15, color: 'var(--color-accent)', width: 18 }, children: c.icone }), _jsx("span", { style: { fontSize: 13, color: 'var(--color-text-primary)' }, children: c.label })] }, c.id))), resultados.length > 0 && (_jsx("div", { style: { padding: '10px 20px 4px', fontSize: 10, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--color-text-muted)', borderTop: comandosFiltrados.length ? '1px solid var(--color-border)' : 'none', marginTop: comandosFiltrados.length ? 4 : 0 }, children: "Blocos" })), resultados.map((r, idx) => {
                            const i = comandosFiltrados.length + idx;
                            return (_jsxs("button", { onClick: () => irBloco(r.resumo_id), onMouseEnter: () => setSel(i), style: {
                                    width: '100%',
                                    display: 'block',
                                    textAlign: 'left',
                                    padding: '12px 20px',
                                ...
                                }, children: [_jsx("p", { style: { margin: 0, fontSize: 14, color: 'var(--color-text-primary)', fontWeight: 600 }, children: r.metadata.titulo }), _jsxs("p", { style: { margin: '2px 0 0', fontSize: 12, color: 'var(--color-text-muted)' }, children: [r.metadata.disciplina, " \u00B7 sem ", r.metadata.semestre, " \u00B7 ", r.resumo_id] })] }, r.resumo_id));
                        })] })), termo.trim() && totalItens === 0 && (_jsxs("p", { style: { padding: '16px 20px', margin: 0, fontSize: 13, color: 'var(--color-text-muted)' }, children: ["Nada encontrado para \"", termo, "\"."] })), _jsxs("div", { style: { padding: '8px 20px', display: 'flex', gap: 16, fontSize: 11, color: 'var(--color-text-muted)', borderTop: '1px solid var(--color-border)' }, children: [_jsx("span", { children: "\u2191\u2193 navegar" }), _jsx("span", { children: "\u21B5 abrir" }), _jsx("span", { children: "esc fechar" })] })] }) }));
}
