...
export function bootstrap() {
    ...
    bootPromise = (async () => {
        ...
        if (import.meta.env.DEV) {
            console.info(`[ANIMA] boot · ${rel.total} blocos (${rel.criados} novos, ${rel.atualizados} atualizados, ${rel.inalterados} iguais) · ${banco.total} questões${banco.recarregado ? ' (recarregadas)' : ''} · persistência: ${status.persistente}`);
            if (rel.erros.length)
                console.warn('[ANIMA] erros de ingestão:', rel.erros);
        }
    })();
    return bootPromise;
}
