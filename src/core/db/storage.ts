...
export interface StatusArmazenamento {
  persistente: boolean
  usoMB: number | null
  cotaMB: number | null
...
export async function garantirPersistencia(): Promise<StatusArmazenamento> {
  ...
  let cotaMB: number | null = null
  let percentual: number | null = null
  try {
    if (navigator.storage?.estimate) {
      const est = await navigator.storage.estimate()
      if (est.usage != null) usoMB = +(est.usage / 1_048_576).toFixed(1)
      if (est.quota != null) cotaMB = +(est.quota / 1_048_576).toFixed(0)
      if (est.usage != null && est.quota) percentual = +((est.usage / est.quota) * 100).toFixed(1)
    }
  } catch {
    // ignora
  }
