...
const TABELAS_INCREMENTAIS = new Set([
    'duvidas',
    'checkins',
    'descobertas',
    'provas',
...
export async function importarProgresso(json) {
    const b = json;
    ...
    const tabelas = b.tabelas ? { ...b.tabelas } : {};
    if (!b.tabelas) {
        if (Array.isArray(b.progresso))
            tabelas.progresso = b.progresso;
        if (Array.isArray(b.duvidas))
            tabelas.duvidas = b.duvidas;
    }
    if (!Array.isArray(tabelas.progresso)) {
        return { ok: false, mensagem: 'Backup sem dados de progresso.', restaurados: 0 };
    }
    let restaurados = 0;
    await db.transaction('rw', TABELAS_ALUNO.map((t) => db.table(t)), async () => {
        for (const nome of TABELAS_ALUNO) {
            ...
            if (nome === 'progresso' || nome === 'progressoQuestao') {
                ...
                const atuais = new Map((await db.table(nome).toArray()).map((p) => [
                    p[pk],
                    tsEpoch(p.atualizado_em),
                ]));
                const aGravar = linhas.filter((p) => {
                ...
                const jaTem = new Set(existentes.map((r) => assinatura(r)));
                const novos = linhas
                ...
                if (novos.length)
                    await db.table(nome).bulkAdd(novos);
 ...
 */
export async function snapshotAutomatico(forcar = false) {
    const agora = Date.now();
    if (!forcar && agora - ultimoSnapshot < INTERVALO_SNAPSHOT_MS)
        return;
    ultimoSnapshot = agora;
    try {
        const backup = await exportarProgresso();
        if ((backup.tabelas.progresso?.length ?? 0) === 0)
            return; // nada a salvar ainda
        const snaps = (await getMeta('snapshots')) ?? [];
...
export async function listarSnapshots() {
    return (await getMeta('snapshots')) ?? [];
}
