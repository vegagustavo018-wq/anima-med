...
// Manifesto servido como asset estático (gerado em build/dev por scripts/gerar-manifesto.mjs)
interface EntradaManifesto {
  id: string
  arquivo: string // URL relativa, ex: /blocos/histologia/xxx.json
  hash: string
}
interface Manifesto {
  versao: number
  gerado_em: string
  blocos: EntradaManifesto[]
}

// Manifesto persistido no IndexedDB: id → hash já ingerido
type ManifestoLocal = Record<string, string>
 ...
 */
export async function ingerirBlocos(): Promise<RelatorioIngestao> {
  const rel: RelatorioIngestao = {
    criados: 0,
    atualizados: 0,
    inalterados: 0,
    removidos: 0,
  ...
  const local = (await getMeta<ManifestoLocal>('manifesto_ingestao')) ?? {}
  const novoLocal: ManifestoLocal = { ...local }
  ...
  const idsExistentes = new Set((await db.blocos.toCollection().primaryKeys()) as string[])

  // delta: novo ou hash mudou. Inalterados não fazem I/O nenhum.
  const paraBaixar = manifesto.blocos.filter(
    (e) => !(local[e.id] === e.hash && idsExistentes.has(e.id))
  )
  ...
  for (let i = 0; i < paraBaixar.length; i += CONCORRENCIA) {
    ...
    const resultados = await Promise.all(
      lote.map(async (entrada) => {
        try {
          ...
          if (!resp.ok) throw new Error(`HTTP ${resp.status}`)
          const bloco = (await resp.json()) as unknown
          if (!blocoTemFormaValida(bloco)) throw new Error('bloco com forma inválida')
    ...
    for (const r of resultados) {
      ...
      if (idsExistentes.has(r.entrada.id)) rel.atualizados++
      else rel.criados++
    }
  }
  if (novos.length) await db.blocos.bulkPut(novos)
  ...
  if (removidos.length) {
    ...
    for (const id of removidos) delete novoLocal[id]
  }
  ...
  if (novos.length || removidos.length) invalidarBusca()
  return rel
}
