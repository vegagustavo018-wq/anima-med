...
export interface ValidacaoResultado {
  testes_passados: number
  performance_queries_ms: number[]
  problemas: string[]
  resumo: {
    teste_1_leitura: boolean
    teste_2_busca: boolean
    teste_3_relacionamentos: boolean
    teste_4_abas: boolean
    teste_5_conflitos: boolean
    blocos_lidos: number
    blocos_com_narrativa: number
    blocos_com_flashcards: number
    blocos_com_casos: number
    total_blocos: number
    progresso_registros: number
 ...
 */
export async function testeLeitura(): Promise<TesteResultado & { blocos: number }> {
  ...
  try {
    ...
    // Validação rigorosa
    const todasValidas = amostra.every(
      b =>
        b &&
        b.resumo_id &&
 ...
 */
export async function testeBusca(): Promise<TesteResultado & { encontrados: number }> {
  const inicio = performance.now()
 ...
 */
export async function testeRelacionamentos(): Promise<TesteResultado> {
  const inicio = performance.now()
 ...
 */
export async function testeAbas(): Promise<TesteResultado> {
  const inicio = performance.now()
 ...
 */
export async function testeConflitos(): Promise<TesteResultado> {
  const inicio = performance.now()
  const problemas: string[] = []
  const stats = {
  ...
  try {
    ...
    const blocos = await db.blocos.toArray()
    const idsVistoBlocos = new Set<string>()
    ...
    const idsVistoQuestoes = new Set<string>()
 ...
 */
export async function validarIntegridade(): Promise<ValidacaoResultado> {
  console.log(
    '\n╔════════════════════════════════════════════════════════════╗'
  )
  console.log(
    '║  ANIMA Med — Validador de Integridade (Pós-Migração)      ║'
  )
  console.log(
    '╚════════════════════════════════════════════════════════════╝\n'
  )
  ...
  console.log(
    `[1/5] Leitura: ${t1.passou ? '✅' : '❌'} | ${t1.ms.toFixed(2)}ms | ${t1.blocos} blocos`
  )

  const t2 = await testeBusca()
  console.log(
    `[2/5] Busca: ${t2.passou ? '✅' : '❌'} | ${t2.ms.toFixed(2)}ms | ${t2.encontrados} encontrados`
  )
  ...
  const testes_passados = [t1.passou, t2.passou, t3.passou, t4.passou, t5.passou].filter(
    Boolean
  ).length
  ...
  const resultado: ValidacaoResultado = {
    ...
    resumo: {
      ...
      blocos_lidos: (t1 as any).blocos || 0,
      blocos_com_narrativa: (t4.dados as any)?.blocos_com_narrativa || 0,
      blocos_com_flashcards: (t4.dados as any)?.blocos_com_flashcards || 0,
      blocos_com_casos: (t4.dados as any)?.blocos_com_casos || 0,
      total_blocos: (t4.dados as any)?.total_blocos || 0,
  ...
  console.log(
    `║  RESULTADO: ${resultado.testes_passados}/5 testes passados              ${' '.repeat(12)} ║`
  )
  console.log(
    `╚════════════════════════════════════════════════════════════╝\n`
  )
