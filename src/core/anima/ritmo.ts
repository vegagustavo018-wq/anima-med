...
export interface DiaRitmo {
  ...
  intensidade: number // 0-1, quanto foi tocado o organismo naquele dia
  ehHoje: boolean
  ehRetomada: boolean // primeiro dia de atividade após 2+ dias de silêncio
}
 ...
 */
export async function calcularOnda(): Promise<DiaRitmo[]> {
...
export function continuidadeAtual(dias: DiaRitmo[]): number {
  let n = 0
  for (let i = dias.length - 1; i >= 0; i--) {
    if (dias[i].intensidade > 0) n++
    else if (!dias[i].ehHoje) break
    else continue // hoje sem atividade ainda não quebra
  }
  return n
}
