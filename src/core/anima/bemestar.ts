...
export interface LeituraBemEstar {
  sobrecarga: boolean
  motivo: string | null
  sessoesTardias7d: number
  vencidosAcumulados: number
}
 ...
 */
export async function lerBemEstar(): Promise<LeituraBemEstar> {
  ...
  for (const p of progresso) {
    for (const h of p.historico_revisoes) {
      ...
      if (d < seteDiasAtras) continue
      const hora = d.getHours()
      if (hora >= 0 && hora < 5) sessoesTardias7d++
    }
  }
