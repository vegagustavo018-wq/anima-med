...
import type { BlocoConteudo, StatusEstudo } from '@core/types/schema'
...
export interface NoGrafo {
  ...
  x: number
  y: number
  status: StatusEstudo | 'novo'
  grau: number // quantidade de conexões — define o tamanho visual
}

export interface ArestaGrafo {
  de: string
  para: string
  tipo: 'arvore' | 'prerequisito' | 'futura' | 'lateral'
}

export interface GrafoGlobal {
  nos: NoGrafo[]
  arestas: ArestaGrafo[]
  disciplinas: string[]
}
 ...
 */
export async function construirGrafoGlobal(): Promise<GrafoGlobal> {
  ...
  const grauPorId = new Map<string, number>()
  const inc = (id: string) => grauPorId.set(id, (grauPorId.get(id) ?? 0) + 1)
  ...
  const porDisciplina = new Map<string, BlocoConteudo[]>()
  for (const b of blocos) {
