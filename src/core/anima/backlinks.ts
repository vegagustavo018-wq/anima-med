...
export interface Backlink {
  resumo_id: string
  titulo: string
  tipo: 'prerequisito' | 'lateral' | 'futura' | 'filho' | 'pai'
}
 ...
 */
export async function backlinksDe(resumo_id: string): Promise<Backlink[]> {
  const blocos = await db.blocos.toArray()
  const alvo = blocos.find((b) => b.resumo_id === resumo_id)
  const out: Backlink[] = []

  for (const b of blocos) {
    if (b.resumo_id === resumo_id) continue
  ...
  if (alvo?.no_pai_id) {
    ...
    if (pai) out.push({ resumo_id: pai.resumo_id, titulo: pai.metadata.titulo, tipo: 'pai' })
  }
  for (const filhoId of alvo?.nos_filhos_ids ?? []) {
    const filho = blocos.find((b) => b.resumo_id === filhoId)
    if (filho) out.push({ resumo_id: filho.resumo_id, titulo: filho.metadata.titulo, tipo: 'filho' })
  }
