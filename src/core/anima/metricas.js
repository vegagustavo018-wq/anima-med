...
export async function calcularMetricas() {
    ...
    const dominadoStatus = (s) => s === 'dominado' || s === 'revisando';
    // por disciplina
    const discMap = new Map();
    for (const b of blocos) {
        const d = b.metadata.disciplina;
        if (!discMap.has(d))
            discMap.set(d, { disciplina: d, total: 0, iniciados: 0, dominados: 0, percentual: 0 });
        const m = discMap.get(d);
        m.total++;
        const p = progById.get(b.resumo_id);
        if (p && p.vezes_lido > 0)
            m.iniciados++;
        if (p && dominadoStatus(p.srs.status))
            m.dominados++;
    }
    for (const m of discMap.values())
        m.percentual = m.total ? Math.round((m.dominados / m.total) * 100) : 0;
    ...
    const diasComAtividade = new Set(eventos
    ...
    for (const p of progresso)
        if (p.ultima_leitura)
            diasComAtividade.add(p.ultima_leitura.slice(0, 10));
    let streak = 0;
    ...
    const sessoesSemana = new Set(progresso.flatMap((p) => p.calibracao.filter((c) => c.data >= seteDias).map((c) => c.data.slice(0, 13)))).size;
