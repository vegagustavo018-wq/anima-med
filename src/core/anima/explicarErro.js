 ...
 */
export async function explicarErro(bloco) {
    const prereqs = bloco.conexoes.prerequisitos;
    if (prereqs.length > 0) {
        const progressos = await Promise.all(prereqs.map((p) => db.progresso.get(p.bloco_id)));
        const forcaDe = (p) => !p ? -1 : p.srs.status === 'dominado' ? 3 : p.srs.status === 'revisando' ? 2 : p.srs.status === 'aprendendo' ? 1 : 0;
        let piorIdx = 0;
        for (let i = 1; i < prereqs.length; i++) {
            if (forcaDe(progressos[i]) < forcaDe(progressos[piorIdx]))
                piorIdx = i;
        }
