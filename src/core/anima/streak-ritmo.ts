 ...
 */
export async function registrarSessaoEstudo(sessao: Omit<SessaoEstudo, 'id' | 'criado_em'>): Promise<SessaoEstudo> {
  ...
  let sessaoSalva: SessaoEstudo

  if (sessaoExistente) {
    // Merge: agregar blocos, somar tempos
    const blocosMap = new Map(
      sessaoExistente.blocos_estudados.map((b) => [b.bloco_id, b])
    )
    ...
    sessaoSalva = {
      ...
      tempo_sessao_ativa_minutos:
        (sessaoExistente.tempo_sessao_ativa_minutos || 0) + (sessao.tempo_sessao_ativa_minutos || 0),
      qualidade_flashcards_media:
        (sessaoExistente.qualidade_flashcards_media + sessao.qualidade_flashcards_media) / 2,
 ...
 */
async function atualizarDiaEstudo(data: string): Promise<void> {
  ...
  const blocos_unicos = new Set(
    sessoes.flatMap((s) => s.blocos_estudados.map((b) => b.bloco_id))
  ).size

  const qualidade_media =
    sessoes.reduce((sum, s) => sum + s.qualidade_flashcards_media, 0) / sessoes.length

  const intensidade = computarIntensidade(qualidade_media, tempo_total)

  const diaEstudo: DiaEstudo = {
    data,
 ...
 * - Registrar a quebra
 */
async function recalcularStreak(): Promise<void> {
  ...
  let dias_atuais = 0
  let recorde = 0
  let ultima_sessao_data: string | null = null
  let quebrarom_em: string | null = null
  ...
  for (let i = 0; i < historicoReverso.length; i++) {
    ...
    if (dia.tem_dados) {
      dias_atuais++
      if (!ultima_sessao_data) ultima_sessao_data = dia.data
    } else {
      // Quebra encontrada
      if (dias_atuais > 0) {
  ...
  if (recorde === 0) recorde = dias_atuais

  const dias_desde_ultima = ultima_sessao_data
    ? Math.floor(
      (new Date(hoje).getTime() - new Date(ultima_sessao_data).getTime()) /
        (1000 * 60 * 60 * 24)
    )
    : 999
 ...
 */
export async function obterStreakAtual(): Promise<Streak | null> {
 ...
 */
export async function gerarHeatmap(): Promise<CelHeatmap[]> {
  const hoje = new Date()
  const inicio = new Date(hoje)
  inicio.setDate(hoje.getDate() - 83) // 12 semanas atrás

  const diasIntervalo: string[] = []
  for (let i = 0; i < 84; i++) {
  ...
  const diasEstudoMap = new Map<string, DiaEstudo>()
 ...
 */
export async function gerarResumoRitmo(): Promise<ResumoRitmo> {
  ...
  let proximaMeta: Milestone | null = null
  ...
  if (mediaAtual > mediaAnterior + 0.5) tendencia = 'subindo'
  else if (mediaAtual < mediaAnterior - 0.5) tendencia = 'caindo'
  ...
  const dias_desde_ultima = ultimaSessao
    ? Math.floor(
      (new Date().getTime() - new Date(ultimaSessao.data).getTime()) /
        (1000 * 60 * 60 * 24)
    )
    : 999
 ...
 */
export async function detectarRiscoQuebra(): Promise<number | null> {
  const streak = await obterStreakAtual()
  if (!streak || streak.dias_atuais < 7) return null // sem risco ainda
  ...
  return predizerRiscoQuebra(
    streak.dias_atuais,
    streak.ultima_sessao_data || new Date().toISOString().split('T')[0],
    historicoUltimas2sem
  )
}
 ...
 */
export async function obterMilestonesHistoricos(): Promise<Milestone[]> {
  const streak = await obterStreakAtual()
  if (!streak) return []
...
/**
 * Exports para integração com EstudarPage e ProgressoPage.
 */
export type { SessaoEstudo, DiaEstudo, Streak, ResumoRitmo, CelHeatmap, Milestone, FaseRitmo }
