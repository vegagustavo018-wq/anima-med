...
import type { EventoRevisao } from '@core/types/schema'
...
export interface ErroClassificado {
  ...
  titulo: string
  disciplina: string
  tipoCard: string | null // tipo do flashcard, se o erro foi num card específico
}
...
export interface DiagnosticoErros {
  totalErros: number
  porDisciplina: PadraoErro[]
  porTipoCard: PadraoErro[]
  recentes: ErroClassificado[]
  fraseDiagnostico: string | null
}
 ...
 */
export async function diagnosticarErros(): Promise<DiagnosticoErros> {
  ...
  const erros: ErroClassificado[] = []

  for (const p of progresso) {
    const bloco = blocoPorId.get(p.resumo_id)
    if (!bloco) continue
    for (const h of p.historico_revisoes as EventoRevisao[]) {
      if (h.qualidade >= 3) continue
      let tipoCard: string | null = null
      if (h.alvo !== 'bloco') {
  ...
  const contarPor = (fn: (e: ErroClassificado) => string | null): PadraoErro[] => {
  ...
  let fraseDiagnostico: string | null = null
