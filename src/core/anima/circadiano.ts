...
export interface CelulaCircadiana {
 ...
 */
export async function calcularCircadiano(): Promise<CelulaCircadiana[]> {
  const progresso = await db.progresso.toArray()
  const grade = new Map<string, CelulaCircadiana>()
...
export function melhorFaixa(celulas: CelulaCircadiana[]): string | null {
