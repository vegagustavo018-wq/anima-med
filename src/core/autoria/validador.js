export function validarSchema(b) {
    const p = [];
    const req = (cond, campo, msg, g = 'erro') => {
        if (!cond)
            p.push({ campo, mensagem: msg, gravidade: g });
    };
...
export function checar8Etapas(b) {
    ...
    const anti = [];
    const temContrafactual = tipos.has('contrafactual') || (b.flashcards ?? []).some((f) => f.tipo === 'contrafactual');
    if (!temContrafactual)
        anti.push('sem contrafactual');
    const temClinico = (b.flashcards ?? []).some((f) => f.tipo === 'clinico');
    if (!temClinico && (b.casos_clinicos?.length ?? 0) === 0)
        anti.push('sem elemento clínico');
    const flashDefinicao = (b.flashcards ?? []).some((f) => /^(o que é|defina|cite os tipos)/i.test(f.pergunta));
    if (flashDefinicao)
        anti.push('flashcard de definição seca ("O que é X?")');
...
export function checarIntegridade(blocos) {
    ...
    for (const b of blocos) {
        ...
        for (const f of b.nos_filhos_ids ?? []) {
            if (!ids.has(f))
                problemas.push({ resumo_id: b.resumo_id, tipo: 'filho_inexistente', alvo: f });
        }
        for (const l of b.conexoes_laterais_ids ?? []) {
            if (!ids.has(l))
                problemas.push({ resumo_id: b.resumo_id, tipo: 'conexao_orfa', alvo: l });
        }
    }
    return problemas;
}
